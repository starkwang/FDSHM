...
function render(req, res) {
    var templateData = {}
    service.item.get(req.params.pubTimeStamp).then(function(item) {
        console.log(item);
        if (item) {
            templateData.session = req.session ? req.session : {};
            templateData.detailBox = item;
            templateData.detailBox.categoryEn = nameFliter.ch2en(item['category']);
            ...
            // }
            return service.user.getUserByUserId(item['publisher_id']);
        } else {
            return false;
        }
    }).then(function(user) {
